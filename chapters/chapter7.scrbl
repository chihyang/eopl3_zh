...
这些条件确保每个操作符都作用于正确类型的操作数。因此，我们说违反这些条件是@emph{类型错误}
(@emph{type error})。
...
@definition[#:title #f #:tag "d7.1.1"]{
 性质@exact-elem{“}表达值v的类型为t@exact-elem{”}由对t进行归纳得到：
...
@tt{pair}表达式生成一个序对，@tt{unpair}表达式（类似@exercise-ref{ex3.18}）将两个变量绑定到表
...
为@tt{car}和@tt{cdr}写出类似的规则，扩展检查器，处理这些和上述表达式。用@exercise-ref{ex7.8}
...
@tt{type-of}的定义如图7.6-7.8所示。对每个表达式，我们递归处理子表达式，一路传递
...
新编号，使之从@tt{ty1}开始。然后，我们就能用@tt{equal?}比较重新编号的类型（图
7.10-7.11）。

要逐个命名所有未知变量，我们用 @tt{canonical-subst} 生成代换式组。用 @tt{table}
做累加器，直接递归即可。@tt{table} 的长度告诉我们已找出多少个不同的未知类型，我
们可以用其长度给@exact-elem{“}下一个@exact-elem{”}@tt{ty}符号编号。这与图4.1中
使用的 @tt{length} 类似。
