...
义如下：@eopl-index["Regular expression"]
...
@eopl-index[#:range-mark 'start "Tokens"]
...
@eopl-index[#:range-mark 'end "Tokens"]
...
终结符号）、右边（@${rhs\mbox{-}item} 的列表），以及生成式名字。生成式的右边是符
...
@tt{arbno} 关键字即语法中的克莱尼星号：它匹配重复任意次数的条目。例如，生成式
...
我们可以把非终结符序列放入 @tt{arbno} 中。这时，节点中会有多个字段，每个对应一个
...
类似于 @tt{arbno}，我们可以在 @tt{separated-list} 关键字中放置任意非终结符序列。
这时，节点中会有多个字段，每个对应一个非终结符；每个字段包含一个语法树列表。这和
@tt{arbno} 生成的数据完全相同；不同的只是具体语法。

我们偶尔会嵌套 @tt{arbno} 和 @tt{separated-list}。@tt{arbno} 内的非终结符生成一
个列表，所以 @tt{arbno} 内的 @tt{arbno} 内的非终结符生成列表的列表。
