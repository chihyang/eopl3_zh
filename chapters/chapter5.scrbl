...
但是，用大多数过程式语言做这种翻译都很困难：它们不是只在必要时才扩大控制上下文，而
是在每个函数调用处扩大控制上下文（即堆栈！）。由于我们的系统中，过程调用在计算结束
之前决不返回，系统的堆栈将一直增高，直到那时为止。

@elemtag["imperative-lang"]{这种行为不无道理}：在这种语言中，几乎所有的过程调用
都出现在赋值语句的右边，所以，几乎每个过程调用都要扩大控制上下文，以便记住待做的
乘法。因此，系统结构为这种最常见的情况进行了优化。而且，大多数语言在堆栈中存储环
境信息，所以，每个过程调用生成的控制上下文还要记住移除堆栈上的环境信息。
...
的程序可能太过庞大，以至编译器无法处理。把本节的解释器翻译为指令式语言，用
@exercise-ref{ex5.26}中的跳跃技术避免这种困难。
