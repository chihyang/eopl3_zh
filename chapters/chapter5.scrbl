...
在调用时继续计算。整个计算由一个名叫@term["trampoline"]{跳床} 的过程驱动，它从一
个过程调用弹射到另一个。例如，我们可以在 @tt{apply-procedure/k}的主体周围插入一
个 @tt{(lambda () ...)}，因为在我们的语言中，只要不执行过程调用，表达式的运行时
间就是有限的。
...
线程调度由@term["scheduler"]{调度器} 执行，就绪队列为其状态的一部分。
@eopl-index["Scheduler"]
此外，它保存一个计时器，当一个线程执行若干步骤（即线程的@term["time slice"]{时间片}
@eopl-index["Time slice"]
或@term["quantum"]{量子}）时，它中断线程，将其放回就绪队列，并从就绪队列中选出一
个新的线程来运行。这叫做@term["pre-emptive scheduling"]{抢占式调度}。
...
它时才运行。@tt{spawn} 的执行只求效果；在我们的系统中，我们为它任选一个返回值 73。
