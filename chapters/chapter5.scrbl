...
(@emph{operand position})调用。我们需要保存这个调用的语境，因为我们需要记住，过
程调用执行完毕之后，我们仍需求出操作数的值，并执行外层调用，在本例中，是完成待做
的乘法。这给出一条重要原则：
...
的求值扩大了控制语境。具体来说，如果我们在某个续文@${cont_1}中求过程调用
...
给图5.4-5.6的解释器添加辅助过程，生成类似@pageref{cps-computation}计算的输出。
...
都出现在赋值语句的右边，所以，几乎每个过程调用都要扩大控制语境，以便记住待做的乘
法。因此，系统结构为这种最常见的情况进行了优化。而且，大多数语言在堆栈中存储环境
信息，所以，每个过程调用生成的控制语境还要记住移除堆栈上的环境信息。
...
一个@${Bounce}。（在 @secref{s7.4}我们将看到如何自动完成这个推理过程。）
...
的程序可能太过庞大，以至编译器无法处理。把本节的解释器翻译为指令式语言，用练习
5.26中的跳跃技术避免这种困难。
