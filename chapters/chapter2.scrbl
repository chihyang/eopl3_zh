...
@exercise[#:level 2 #:tag "ex2.3"]{
 ...
 @itemlist[#:style 'ordered
   ...
   @item{实现这种整数表示法：写出@pageref{nat}指定的 @tt{zero}，@tt{is-zero?}，
...
哪种lambda演算表达式，并提取出各部分。@pageref{lc-exp}的数据类型@tt{lc-exp}使我
们轻松完成这两项。
...
算表达式指定了一个列表集合，如同@pageref{define-datatype}的@tt{define-datatype}
语法。这样，Scheme@elem[#:style question]{过程}@tt{read}自动把字符串解析为列表和
符号。然后，把这些列表结构解析为抽象语法树就容易多了，就像@tt{parse-expression}
这样。
...
当具体语法使用克莱尼星号或加号（@pageref{kleene-star}）时，设计抽象语法树最好用
@${list}表示相应子树。例如，如果lambda演算表达式的语法为：
