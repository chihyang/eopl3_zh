...
如，考虑@figure-ref{fig-9.4}。类@tt{c2}的对象有两个名为@tt{y}的字段：@tt{c1}中声明的和@tt{c2}中
...
要解释这种区别，考虑@figure-ref{fig-9.6}。给类@tt{c3}的对象@tt{o3}发送消息@tt{m3}，找到的是
...
不是表达值：它们作为对象的一部分，但不能做变量的绑定或表达式的值，不过，看看@exercise-ref{ex9.29}
