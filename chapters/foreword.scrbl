...
@centered{@emph{计算机语言的解释器不过是另一个程序}。}
...
得我可能得推翻语言设计者的一些决定。我不知道如何创造嵌合的子语言来帮我组织系统实
...
@section[#:style section-title-style-unumbered #:tag "fw-s"]{三版附言}

上述序言写于短短七年之前。在九十年代，难以想象七年之内，信息应用和服务已遍布全球。
它们由持续增长的编程语言和编程框架驱动——这一切都基于不断壮大的解释器平台。

想创建Web页面吗？在九十年代，那等于格式化静态文本和图像，实际上是创建由浏览器运
行的程序，而浏览器仅仅执行一条@exact-elem{“}print@exact-elem{”}语句。今天的动
态网页充分利用脚本语言（解释性语言的另一名字），如Javascript。浏览器程序则相当复
杂，包含对Web服务器的异步调用。而服务器通常运行着另一程序，由完全不同的编程框架
写就。框架又可能含有多种服务，每种服务都有各自的语言。
...
又或者你在为传感器网络开发新的算法，并且尝试用惰性求值更好地解决并行和数据汇集问
题。又或者你在探索XSLT那样的转换系统来控制Web页面。又或者你在设计转换和重混多媒
体流的框架。又或者……
