...
@itemlist[#:style 'ordered
 ...
 @item{我们需要一种更好的方式来控制名字的作用域和绑定。词法定界是控制命名的有
...
m1 take b}和@tt{from m1 take c}绑定到适当的值，模块定义之后在它们的作用域内。
由于@tt{from m1 take x}未在接口中声明，所以模块定义之后不在它的作用域内。
...
类型异常，因为@tt{m2}主体中使用@tt{from m1 take u}之处不在后者的作用域内。
}
}
...
我们语言的定界规则很简单：在模块中，按 @tt{let*} 式定界，依次进入绑定的受限变量
的作用域。接口告诉我们每个受限变量的类型。声明和定义也都遵循 @tt{let*} 式定界
（见@figure-ref{fig-8.2}）。
...
是在之前声明的所有模块的作用域中，只是在自身@tt{depends-on}语句中列出模块的作用
域中。例如，考虑程序
...
@tt{m5}的主体仅在来自@tt{m1}或@tt{m3}的受限变量的作用域中。使用@tt{from m4
...
们之前说的，这是因为接口中声明的作用域局限于接口，不包含模块主体。
