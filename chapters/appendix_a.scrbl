...
我们的口号@emph{遵循语法}基于@emph{结构化归纳法}，由 Brustall (1969) 提出。@emph{子目标归纳}
(@emph{subgoal induction}) (Morris &Wegbreit, 1977) 是证明递归过程正确性的有效方
法，即使过程没有@bold{遵循语法}。过程的可能输入受不变式约束时，子目标归纳也有效。
...
图3.13那样的等深线用来解释词法作用范围，首先由 Johnston (1971) 提出。无名解释器
...
个待定语言的程序翻译为定义其行为的数学对象。Plotkin (1977) 对这种技术做了无可替
代的介绍，Winskel (1993) 做了更宽泛的探讨。Milne & Strachey (1976) 是本百科全书，
研究了如何用这种技术建模大量语言特性，

另一种方式是写出待定语言子集的解释器。例如，@secref{state}的几个解释器依靠
...
通用模型。在函数式语言 Haskell (Peyton Jones, 2001) 中，monads 为非函数式行为提
供了组织原则。
...
做 CPS 变换，然后以数据结构表示法表示续文。得出的程序像我们的寄存程序一样，很容
易编译。这条线发展出了 ORBIT 编译器 (Kranz et al., 1986) 和 Standard ML of New
...
(1998)，而对严肃的读者，Gosling et al. (1996)是其规范。
...
效基础。Flatt et al. (1998) 形式化了 Java 的一个子集。另一有用的子集是
...
(Hailpern, 2007) 三次举办了会议，专门讨论编程语言的历史。这些会议囊括了讨论各种
编程语言历史的论文。IEEE Annals of the History of Computing 囊括了介绍方方面面计
