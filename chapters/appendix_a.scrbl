...
@figure-ref{fig-3.13} 那样的等深线用来解释词法作用域，首先由 Johnston (1971) 提
出。无名解释器和翻译器基于德布鲁金索引 (de Bruijn, 1972)。
...
个被定语言的程序翻译为定义其行为的数学对象。Plotkin (1977) 对这种技术做了无可替
代的介绍，Winskel (1993) 做了更宽泛的探讨。Milne & Strachey (1976) 是本百科全书，
研究了如何用这种技术建模大量语言特性，
...
EXPLICIT-REFS 的设计基于 ML (Milner et al., 1989) 的存储器模型，而后者与 Bliss
...
做 CPS 变换，然后以数据结构表示续文。得出的程序像我们的寄存程序一样，很容易编译。
这条线发展出了 ORBIT 编译器 (Kranz et al., 1986) 和 Standard ML of New Jersey 编
译器 (Appel & Jim, 1989)。
...
见 Felleisen & Friedman (1996)，另见 Paulson (1996); Smith (2006)。
...
的模块系统。我们的模块系统基于 Leroy (1994)，它应用于 CAML（参见 Smith, 2006），
而这是 ML 的另一变体。

通常视 Simula 67 (Birtwistle et al., 1973) 为第一种面向对象语言。面向对象的类比
由 Smalltalk (Goldberg & Robson, 1983) 和 Actors (Hewitt, 1977) 扩充。二者都使用
人类互动以及收发消息的类比来解释他们的思想。Sussman 和 Steele 原打算用 Scheme 理
解 Hewitt 的 actor 模型，但 Scheme 超越其原意。Abelson & Sussman (1985, 1996) 和
Springer & Friedman (1989) 给出了更多用 Scheme 进行面向对象编程的例子，讨论了函
数式编程和命令式编程在哪些时候最适合。Steele (1990) 和 Kiczales et al. (1991) 描
述了 Common Lisp 中强大的面向对象编程组件 CLOS。

@secref{oac}的语言基于 Java 的对象模型。Java 的标准参考书是 Arnold & Gosling
(1998)，有意精研的读者可以阅读其规范 Gosling et al. (1996)。
...
Gamma et al. (1995) 编写了一本引人注目的手册，专论编写面向对象程序的有效组织原则。

ACM 于 1978 年 (Wexelblatt, 1978)、1996 年 (Bergin & Gibson, 1996) 和 2007 年
(Hailpern, 2007) 三次举办会议，专门讨论编程语言的历史。这些会议囊括了讨论各种编
程语言历史的论文。IEEE Annals of the History of Computing 囊括了介绍方方面面计算
历史的学术性文章，也包括编程语言。Knuth & Pardo (1977) 介绍了初期编程语言的迷人
历史。
