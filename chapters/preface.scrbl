...
@itemlist[@item{每一新概念都用一种小型语言解释。这些语言通常是递进的：后出语言依
赖于先出语言的特性。}
...
增的，因为我们选择了一阶组合式续文传递风格变换而不是@elem[#:style question]{相关
的另一种}。同时，由于曳尾表达式的性质，我们在这一章使用多参数过程。在对象和类一
章同样如此，但这并非必然。每章都经过修订，新增了很多习题。
...
何用基于@elem[#:style question]{并集}的类型推导算法推断程序中的类型。
...
练习 0.1 [@${\star\star\star}] 我们常说“某语言具有某属性”。为每种说法找出一种
或多种具有该属性的语言，以及一种或多种不具有该属性的语言。请随意搜索这些信息，不
拘任何编程语言的介绍性书籍（比如Scott(2005)，Sebesta (2007)，或者Pratt &
