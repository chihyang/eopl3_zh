...
的源语言假定只能给函数传递一个参数；我们增加了支持多参数过程的
习题。@secref{cps}是新增的，因为我们选择了一阶组合式续文传递风格变换，
而非@elem[#:style question]{关系式的}。同时，由于@emph{尾式} (@emph{tail-form
expression}) 的性质，我们在这一章使用多参数过程。在对象和类一章同样如此，但这并
非必然。每章都经过修订，新增了很多习题。
...
（标为 @make-marker[1]）；困难者，需要花费大量思考和编程工作（标为
