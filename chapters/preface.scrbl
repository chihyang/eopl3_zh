...
@subsection[#:style 'unnumbered #:tag "pf-obj"]{目标}

本书是对编程语言的分析性研究。我们的目标是对编程语言的基本概念提供深入有效的理解。
这些概念的重要性久经证实。它们是理解编程语言未来发展的基础。

这些概念多关乎程序元素的语义，或称含义。含义反映了元素在程序执行时应当如何解释。
名为解释器的程序最直接地表现了程序语义，且能执行。它们直接分析程序文本的抽象表示，
从而处理程序。因此我们以解释器为主要手段，以便表达编程语言元素的语义。
...
@itemlist[ @item{每一新概念都用一种小型语言解释。这些语言通常是递进的：后出语言
依赖于先出语言的特性。}

@item{用语言处理器——如解释器和类型检查器——解释指定语言所写程序的行为。它们以形式
  化（无歧并且完备）和可执行的方式表现语言设计中的决定。}

@item{适当的时候，我们用接口和规范建立数据抽象。如此，我们可以改变数据的表示而不
  必更改程序。我们以此来研究不同的实现策略。}

@item{我们的语言处理器在较高层次写就，用以简括地表示语义；同时也在极低层次写就，
  用以理解实现技巧。}

@item{我们展示了如何用简单的代数操作预测程序行为，并获得它们的属性。但是通常，我
  们很少采用数学符号，而是偏爱研究程序的行为，它们是我们语言实现的组成部分。}

@item{正文解释关键概念，习题探讨备选设计和其它问题。例如，正文阐述静态绑定，习题
  讨论动态绑定。有一支习题将词法定界的概念用于本书设计的各种语言。}]

应用多层次抽象，我们提出了看待编程语言的不同观点。我们的解释器常常提供高层次观点，
...
增的，因为我们选择了一阶组合式续文传递风格变换而不是**相关的另一种**。同时，由于
曳尾表达式的性质，我们在这一章使用多参数过程。在对象和类一章同样如此，但这并非必
然。每章都经过修订，新增了很多习题。

@subsection[#:style 'unnumbered #:tag "pf-org"]{组织}

前两章为细致研究编程语言奠定了基础。第1章强调递推型数据规范和递归编程之间的联系，
介绍了关于变量作用域的一些思想。第2章介绍了数据类型工具，由此引出关于数据抽象的
讨论和表达形式转换的例子，这种转换将在后续章节使用。

第3章使用这些基础描述编程语言的行为。本章介绍了解释器，作为解释语言运行时行为的
...
何用基于**并集**的类型推导算法推断程序中的类型。

第8章建立类型模块，这极度依赖对前一章的理解。模块既使我们能够建立和强化抽象边界，
又提供了新型的定界方式。

第9章以类为中心，展示了面向对象编程语言的基本概念。我们首先设计了高效的运行时结
构，作为本章第二部分内容的基础。第二部分将第7章的类型检查器思想和第一部分的面向
对象编程语言结合起来，结果便是传统的有类型面向对象语言。这要求引入新的概念，包括
接口，抽象方法和转换。

“阅读更多”解释了本书的每一思想源于何处。虽然有时我们只列出了能接触到的来源，读
者泛览本章，仍能窥源访本。

最后，附录B介绍了我们的SLLGEN解析系统。
...
@subsection[#:style 'unnumbered #:tag "pf-use"]{使用}
...
（标为[*]），困难者，须花费大量思考和编程工作（标为[***]）。大量关于应用，历史以
及理论的材料潜藏其间。我们建议读一读每道习题，想一想如何解决它们。虽然我们用
Scheme编写解释程序和转换系统，任何支持一等过程和赋值的语言（ML, Common Lisp,
Python, Ruby等等）都足以完成本书练习。

练习 0.1 [*] 我们经常使用这样的短语，如“某语言具有某属性”。对每个这样的短语，
找出一种或多种具有该属性的语言，以及一种或多种不具有该属性的语言。请随意搜索这些
信息，不拘任何编程语言的介绍性书籍（比如Scott(2005), Sebesta (2007)，或者Pratt &
Zelkowitz(2001)）。
