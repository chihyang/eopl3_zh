...
章的源语言假定只能给函数传递一个参数；我们增加了支持多参数过程的习题。
@secref{cps}是新增的，@elem[#:style question]{因为我们选择了一阶组合式续文传递风
格变换，而非相关的另一种}。同时，由于@emph{曳尾} (@emph{tail-form}) 表达式的性质，
...
之间的联系，介绍了关于变量作用范围的一些思想。@secref{da}介绍了数据类型工具，由
此引出关于数据抽象的讨论和表达形式转换的例子，这种转换将在后面的章节中使用。

@secref{expr}使用这些基础描述程序语言的行为。本章介绍了解释器，作为解释语言运行时行为的
机制，并为一门简单语言设计了解释器，该语言支持静态定界、一等过程和递归。这个解释
器是本书其余许多材料的基础。本章结束时，细致讨论了一种用索引代替变量的语言，因此，
这语言可通过列表引用来查询变量。

@secref{state}介绍了新的部分，状态，用以将位置对应到值。一旦引入状态，我们可以研
究关于表示的各种问题。此外，它使我们能够探索参数传递机制：按址调用、按名调用、以
及按需调用。
...
@secref{cps}是上一章的姊妹篇。上一章我们展示了如何把我们熟悉的解释器转换为续文传
递风格；本章我们展示了如何为更大的一类程序完成转换。续文传递风格是一种强大的编程
工具，因为它使所有顺序控制机制能在几乎任何语言中实现。这一算法也是源到源程序转换
抽象描述的例子。
...
@secref{oac}以类为中心，展示了面向对象程序语言的基本概念。我们首先设计了高效的运
行时结构，作为本章第二部分内容的基础。第二部分将@secref{types}的类型检查器思想和
第一部分的面向对象程序语言结合起来，结果便是传统的有类型面向对象语言。这要求引入
新的概念，包括接口，抽象方法和转换。

“@elemref["readings"]{阅读更多}”解释了本书的每一思想源于何处。虽然有时我们只列
出了能接触到的来源，读者泛览本章，仍能窥源访本。
...
成。@note{本书网站已迁移至@url{http://www.eopl3.com}，代码改用Racket实现，网址为
@url{https://github.com/mwand/eopl3}。——@emph{译注}}我们选择这种Scheme实现，因为
它的模块系统和编程环境对学生助益良多。代码多半兼容于R@superscript{5}RS，当能轻易
移植到任何功能完整的Scheme实现。
