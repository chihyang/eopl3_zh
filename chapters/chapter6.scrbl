...
曳尾表达式的传递续文解释器如图6.6所示。由于这种语言的过程取多个参数，我们用
...
我们可以把这种行为装入过程@tt{cps-of-exps}中，如图6.8所示。它用@exercise-ref{ex1.23}中的
...
写一个翻译器，它取@exercise-ref{ex6.32}的输出，将其转换为图6.1那样的寄存器程序。
