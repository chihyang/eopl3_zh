...
推理规则和方程，推导出这样的断言。我们手写规则和方程，找出某些表达式的期望值。

}

而我们的目标是写出程序，实现语言。概况如图3.1(a)所示。首先是程序，由我们要实现的
...
目标语言可能像图3.1(b)那样，由一个解释器执行，也可能为执行而翻译成更底层的语言。
...
另一种方式是忽略具体语法的细节，把表达式写成列表结构，就像 @secref{s2.5}和练习
...
图3.2展示了我们这门简单语言的语法。在这种语言中，程序只能是一个表达式。表达式是
...
任何环境中，常量表达式的值都是该常量。某一环境中，变量引用的值从环境中查询而得。
某一环境中，差值表达式的值为第一个操作数在该环境中的值减去第二个操作数在该环境中
的值。当然，准确来说，我们得确保操作数的值是数字，且结果是表达值表示的数字。
...
当且仅当操作数的值为0，@tt{zero?} 表达式的值为真。像定义 1.1.5 那样，可将其写成
...
见@elemref["sllgen"]{附录B}。——@emph{译注}}作为前端，表达式用图3.6中的数据类型表示。
...
析它，把结果交给 @tt{value-of-program}。最有意思的过程是 @tt{value-of}，它取一表
达式和一环境，用解释器秘方计算规范要求的答案。在代码中，我们插入了相关的推理规则
定义，以便观察 @tt{value-of} 的代码如何与规范对应。
